import { HomogenousVector as Vector } from '../HomogeneousVector'
import { mult, True, False } from '@juan-utils/functions';
import { matchContainer } from '../fp';

describe("HomogenousVector", () => {
    describe("Vector operations", () => {
        it("get x , y , z", () => {
            const v = Vector.of(1,2,3);
            expect([ v.x, v.y, v.z ]).toStrictEqual([1,2,3]);
        })
        it("get should default to 1", () => {
            const v = Vector.from([]);
            expect([ v.x, v.y, v.z ]).toStrictEqual([1,1,1]);
        })
        it("scale", () => {
        ...
        it("add", () => {
            ...
            expect(v0.add(v1).toArray()).toStrictEqual([4,5,4]);
        })
        it("sub", () => {
            const v0 = Vector.of(1,2,3);
            const v1 = Vector.of(3,3);
            expect(v0.sub(v1).toArray()).toStrictEqual([-2,-1,2]);
        })
        it("mult", () => {
            const v0 = Vector.of(1,2,3);
            const v1 = Vector.of(3,3);
            expect(v0.mult(v1).toArray()).toStrictEqual([3,6,3]);
        })
        it("mod", () => {
        ...
        it("distance", () => {
            ...
            const v1 = Vector.of(1,1,0);
            expect(v1.distance(v0)).toBe(1);
        })
        it("elevate", () => {
    ...
    describe("Container", () => {
        it("should match to HomogenousVector", () => {
            expect(matchContainer({
                HomogenousVector: True,
                _ : False
